...
?>
<html>
<title>Import Patient Roster List Here</title>
<form class="form-inline" enctype="multipart/form-data" method='post' name='import' id='import' action="new_leaf_import_script_client_roster.php" onsubmit='return top.restoreSession()'>

    <div class="form-group mb-2">
        <label for="mss_input_file">Patient Roster</label>
        <input type="file" class="form-control-file" id="input_file" name="input_file">
    </div>
    <input type="submit" class="btn btn-primary mb-2" value="Import" \>
</form>
<?php
...
function buildInsuraceDataTable($array, $pid){
    ...
    foreach($array as $key => $val) {
        ...
        } else if ((strpos(trim(strtolower($key)), 'insurance_notes')) !== false) {
            $insArray = explode(' ', $val);
            $patientInsData['date'] = date('Y-m-s', strtotime($insArray[0]));
...
function importInsuranceData($patient_data, $pid, $insData){
    ...
    //does a insurance record exist for this pid?  If so we update, if not we add a promary, secondart and tritary columns
    newInsuranceData(
        $pid,
        "primary",
        $insData['provider'],
...
//read the first line
if($_FILES) {
    $fh_source = fopen($_FILES['input_file']['tmp_name'], 'r') or die("Failed to open file");
