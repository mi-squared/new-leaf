...
//***This is where we set up the files.
//Check to see if the directories exist where we want to send the claims
if($GLOBALS['gen_x12_based_on_ins_co']) {
    $bat_file_dir = array();
    $bat_x12_clearing_house_files = array();
    $query = sqlStatement("Select * from x12_partners where 1");
    while ($rowdir = sqlFetchArray($query)) {
        if (!is_dir($rowdir['x12_sftp_local_dir']) && isset($rowdir['x12_sftp_local_dir'])) {
            $state = mkdir($rowdir['x12_sftp_local_dir'], '644', true);
            $bat_file_dir[$rowdir['x12_sftp_local_dir']] = array();

        } else {
            $bat_file_dir[$rowdir['x12_sftp_local_dir']] = array();
        }

        $bat_x12_clearing_house_files[$rowdir['id']] = array();
    }

    $bat_x12_clearing_house_files['other'] = array();

    //The array we are saving to is $bat_x12_clearing_house_files
    //get the partner number and push a copy of this claim to
    // $bat_x12_clearing_house_files array
    //if this partner does not exist or if there isn't a partner listed, we can put this to
    // ['other']
    foreach ($_POST['claims'] as $claimid => $claim_array) {
        if(isset($claim_array['partner'])){
            $bat_x12_clearing_house_files[$claim_array['partner']][$claimid]= $claim_array ;

        }else{
            $bat_x12_clearing_house_files['other'][$claimid]= $claim_array ;
...
function send_batch()
{
    ...
    //***This where the files names go.
    if($bat_file_dir !== false) {
        if ($GLOBALS['gen_x12_based_on_ins_co']) {

            $fh = @fopen("$bat_file_dir/$bat_filename", 'a');
            if ($fh) {
                fwrite($fh, $bat_content);
                fclose($fh);
            }
...
if($GLOBALS['gen_x12_based_on_ins_co']) {
    foreach($bat_x12_clearing_house_files as $index => $payer) {
        ...
        $newAr = $_POST;
        $newAr['claims'] = $payer;
        //Change the bat_filename
        $bat_filename = date("Y-m-d-Hi", $bat_time) . "-batch.";
        $bat_filename .= (isset($_POST['bn_process_hcfa']) || isset($_POST['bn_process_hcfa_form']) || isset($_POST['bn_process_ub04']) || isset($_POST['bn_process_ub04_form'])) ? 'pdf' : 'txt';
        $bat_filename = str_replace('batch', 'batch-p'.$index, $bat_filename );

        if ($index != 'other') {
            $bat_file_dir = sqlQuery("select * from x12_partners where id = $index")['x12_sftp_local_dir'];

        }else{

            $bat_file_dir = false;
        }
        //get the batfile directory
        process_form_as_clearing_house($newAr);
    }
...
//***This is the process_form that handes the claim as if we were a clearing house
function process_form_as_clearing_house($ar)
{
    global $bill_info, $bat_filename, $pdf, $template;
    global $ub04id, $validatePass;
